...
export class CarsComponent implements OnInit {
	...
	cars: any;
	p = 1;
	car = { id: 0, code: '', name: '', distribution: 0 };
	loading = false;
	isUpdate = false;
	...
	onSubmit(carForm: any) {
		this.loading = true;
		this.codingService
			.addCar(this.car.id, this.car.code, this.car.name, this.car.distribution)
			.subscribe(
				(res) => {
					if (this.isUpdate) {
						this.cars = res.body;
	...
	getCars() {
		this.codingService.getCars().subscribe((res) => {
			this.cars = res.body;
		});
	}
	...
	editCar(id: number, code: string, name: string, distribution_id: number) {
		this.isUpdate = true;
		this.car.id = id;
		this.car.code = code;
